...
浏览 [Waline的官方接口文档时](https://waline.js.org/reference/server/api.html) 发现通过 `/api/comment?type=recent` 接口能返回网站最新评论数据，于是“奴役”AI给我写了一个适用当前主题的评论展示组件，目前没出啥大差错🤣

## 引入

代码片段Gist开源地址：[Recent Comments for Astro-Theme-Pure](https://gist.github.com/ljxme/e08c5d42abc5e5aa3fdda2d16aa5a60a)

1. 新建`src/plugins/recentcomment.ts`文件并填入以下内容：
...
2. 新建`src/assets/styles/rc.css`文件并填入以下内容：
...
3. 新建`src/components/RecentComments.astro`文件并填入以下内容：
